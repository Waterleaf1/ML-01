...
import time
import re
# import Naverisk_API_Calls as NAC
# from Client_Assets import nav_login
...
def CreateApp(appName,appTemplate,clientName,splunkName,clientAbbrev):
    ...
    # NR = nav_login()
    # CL_content = NR.GetClientsList()
    # data_dict_CL = NAC.ProcessClientsList(CL_content)
    # clientNames = [cn.lower() for cn in data_dict_CL['ClientName']]
    # name_idx = clientNames.index(clientName.lower())
    # ClientID = data_dict_CL['ClientID'][name_idx]
    ClientID = clients_df.loc[clientName.lower(),'id']
    
    # os.system('/opt/splunk/bin/splunk create app {} -template {}'.format(appName,appTemplate.lower()))
